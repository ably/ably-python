...
class Channel(object):
    ...
    def publish_array(self, name, data, timeout=None):
        ...
        for i in range(len(data)):
            ...
            if self.ably.options.use_text_protocol:
                ...
                log.warn("WARN, sending as json but supposed to send as binary. Binary not yet implemented.")
                #TODO was as_thrift
                request_body.append(message.as_json())
    ...
    def publish(self, name, data, timeout=None):
        ...
        if self.ably.options.use_text_protocol:
            ...
            log.warn("WARN, sending as json but supposed to send as binary. Binary not implemented yet")
            request_body = message.as_json()
