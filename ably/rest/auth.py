...
class Auth(object):
    ...
    def can_request_token(self):
      if self.__auth_options.keyId and self.__auth_options.keyValue:
          return True
      if self.__auth_options.authUrl:
          return True
      if self.__auth_options.auth_callback:
         return True
      return False
    ...
    def requestToken(self, keyId=None, keyValue=None, query_time=None,
        ...
        if not response.error:
            ...
            log.warn("Failed to fetch token Details.")
            return TokenDetails()
