...
class TestRestHttp(BaseAsyncTestCase):
    ...
    async def test_host_fallback(self):
        ...
        def make_url(host):
            base_url = '%s://%s:%d' % (ably.http.preferred_scheme, host, ably.http.preferred_port)
    ...
    async def test_no_retry_if_not_500_to_599_http_code(self):
        ...
        default_url = '%s://%s:%d/' % (ably.http.preferred_scheme, default_host, ably.http.preferred_port)
    ...
    async def test_500_errors(self):
        ...
        with mock.patch('httpx.Request', wraps=httpx.Request):
            with mock.patch(
                'ably.util.exceptions.AblyException.raise_for_response', side_effect=raise_ably_exception
            ) as send_mock:
                with pytest.raises(AblyException):
                    await ably.http.make_request('GET', '/', skip_auth=True)
    ...
    def test_custom_http_timeouts(self):
        ably = AblyRest(
            ...
            http_open_timeout=8,
            http_max_retry_count=6,
            http_max_retry_duration=20,
        )
