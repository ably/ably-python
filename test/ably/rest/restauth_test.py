...
class TestAuth(BaseAsyncTestCase):
    ...
    def test_with_default_token_params(self):
        ably = AblyRest(key=self.test_vars['keys'][0]['key_str'], default_token_params={'ttl': 12345})
...
class TestAuthAuthorize(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    async def test_authorize_create_new_token_if_expired(self):
        ...
        with mock.patch('ably.rest.auth.Auth.token_details_has_expired', return_value=True):
    ...
    async def test_with_token_str_https(self):
        ...
        token = token.token
        ably = await TestApp.get_ably_rest(
            key=None, token=token, tls=True, use_binary_protocol=self.use_binary_protocol
        )
    ...
    async def test_with_token_str_http(self):
        ...
        token = token.token
        ably = await TestApp.get_ably_rest(
            key=None, token=token, tls=False, use_binary_protocol=self.use_binary_protocol
        )
        await ably.channels.test_auth_with_token_str.publish('event', 'foo_bar')
        await ably.close()

    async def test_if_default_client_id_is_used(self):
        ably = await TestApp.get_ably_rest(client_id='my_client_id', use_binary_protocol=self.use_binary_protocol)
    ...
    async def test_timestamp_is_not_stored(self):
        ...
        auth_options = dict(self.ably.auth.auth_options.auth_options)
        auth_options['auth_headers'] = {'a_headers': 'a_value'}
        token_1 = await self.ably.auth.authorize({'ttl': 60 * 1000, 'client_id': 'new_id'}, auth_options)
        assert isinstance(token_1, TokenDetails)

        # call authorize again with timestamp set
        timestamp = await self.ably.time()
        with mock.patch('ably.rest.auth.TokenRequest', wraps=ably.types.tokenrequest.TokenRequest) as tr_mock:
            auth_options = dict(self.ably.auth.auth_options.auth_options)
            auth_options['auth_headers'] = {'a_headers': 'a_value'}
            token_2 = await self.ably.auth.authorize(
                {'ttl': 60 * 1000, 'client_id': 'new_id', 'timestamp': timestamp}, auth_options
            )
    ...
    async def test_client_id_precedence(self):
        ...
        channel = ably.channels[self.get_channel_name('test_client_id_precedence')]
...
class TestRequestToken(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    async def test_with_key(self):
        ...
        ably = await TestApp.get_ably_rest(
            key=None, token_details=token_details, use_binary_protocol=self.use_binary_protocol
        )
    ...
    async def test_with_auth_url_headers_and_params_GET(self):  # noqa: N802
        ...
        def call_back(request):
            ...
            return Response(status_code=200, json={'issued': 1, 'token': 'another_token_string'})

        auth_route.side_effect = call_back
        token_details = await ably.auth.request_token(
            token_params=token_params, auth_url=url, auth_headers=headers, auth_params=auth_params
        )
    ...
    async def test_with_callback(self):
        ...
        token_details = await ably.auth.request_token(token_params=called_token_params, auth_callback=callback)
        assert isinstance(token_details, TokenDetails)
        assert 'token_string' == token_details.token
    ...
    async def test_when_auth_url_has_query_string(self):
        ...
        await ably.auth.request_token(auth_url=url, auth_headers=headers, auth_params={'spam': 'eggs'})
    ...
    async def test_client_id_null_until_auth(self):
        client_id = uuid.uuid4().hex
        token_ably = await TestApp.get_ably_rest(default_token_params={'client_id': client_id})
...
class TestRenewToken(BaseAsyncTestCase):
    async def asyncSetUp(self):
        ...
        self.request_token_route = self.mocked_api.post(
            '/keys/{}/requestToken'.format(self.test_vars['keys'][0]['key_name']), name='request_token_route'
        )
        self.request_token_route.return_value = Response(
            status_code=200,
            headers=headers,
            json={'token': tokens[self.request_token_route.call_count - 1], 'expires': (time.time() + 60) * 1000},
        )
        ...
        def call_back(request):
            ...
            return Response(
                status_code=401,
                headers=headers,
                json={'error': {'message': 'Authentication failure', 'statusCode': 401, 'code': 40140}},
            )

        self.publish_attempt_route = self.mocked_api.post(
            '/channels/{}/messages'.format(self.channel), name='publish_attempt_route'
        )
    ...
    async def test_when_not_renewable_with_token_details(self):
        ...
        self.ably = await TestApp.get_ably_rest(
            key=None, rest_host=self.host, token_details=token_details, use_binary_protocol=False
        )
        await self.ably.channels[self.channel].publish('evt', 'msg')
...
class TestRenewExpiredToken(BaseAsyncTestCase):
    async def asyncSetUp(self):
        ...
        self.mocked_api = respx.mock(base_url='https://{}'.format(self.host))
        self.request_token_route = self.mocked_api.post(
            '/keys/{}/requestToken'.format(key), name='request_token_route'
        )
        self.request_token_route.return_value = Response(
            status_code=200,
            headers=headers,
        ...
        self.publish_message_route = self.mocked_api.post(
            '/channels/{}/messages'.format(self.channel), name='publish_message_route'
        )
        self.time_route = self.mocked_api.get('/time', name='time_route')
        self.time_route.return_value = Response(status_code=200, headers=headers, json=[int(time.time() * 1000)])
