...
class TestRestAppStatsSetup:
    ...
    def get_params(self):
        return {'start': self.last_interval, 'end': self.last_interval, 'unit': 'minute', 'limit': 1}
    ...
    async def asyncSetUp(self):
        ...
        stats = [
            {
                'intervalId': Stats.to_interval_id(self.last_interval - timedelta(minutes=2), 'minute'),
                'inbound': {'realtime': {'messages': {'count': 50, 'data': 5000}}},
                'outbound': {'realtime': {'messages': {'count': 20, 'data': 2000}}},
            },
            {
                'intervalId': Stats.to_interval_id(self.last_interval - timedelta(minutes=1), 'minute'),
        ...
        for i in range(previous_year_stats):
            previous_stats.append(
                {
                    'intervalId': Stats.to_interval_id(self.previous_interval - timedelta(minutes=i), 'minute'),
...
class TestDirectionForwards(TestRestAppStatsSetup, BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    def get_params(self):
        return {
...
class TestDirectionBackwards(TestRestAppStatsSetup, BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    def get_params(self):
        return {'end': self.last_interval, 'unit': 'minute', 'direction': 'backwards', 'limit': 1}
...
class TestOnlyLastYear(TestRestAppStatsSetup, BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    def get_params(self):
        return {'end': self.last_interval, 'unit': 'minute', 'limit': 3}
...
class TestPreviousYear(TestRestAppStatsSetup, BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    def get_params(self):
        return {
...
class TestRestAppStats(TestRestAppStatsSetup, BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    @dont_vary_protocol
    ...
    async def test_when_limit_gt_1000(self):
        params = {'end': self.last_interval, 'limit': 5000}
