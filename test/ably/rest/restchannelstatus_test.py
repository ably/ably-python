...
class TestRestChannelStatus(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    async def test_channel_status(self):
        ...
        assert (
            isinstance(channel_status.status.occupancy.metrics.publishers, int)
            and channel_status.status.occupancy.metrics.publishers >= 0
        ), 'Expected publishers to be a non-negative int'
        assert (
            isinstance(channel_status.status.occupancy.metrics.connections, int)
            and channel_status.status.occupancy.metrics.connections >= 0
        ), 'Expected connections to be a non-negative int'
        assert (
            isinstance(channel_status.status.occupancy.metrics.subscribers, int)
            and channel_status.status.occupancy.metrics.subscribers >= 0
        ), 'Expected subscribers to be a non-negative int'
        assert (
            isinstance(channel_status.status.occupancy.metrics.presence_members, int)
            and channel_status.status.occupancy.metrics.presence_members >= 0
        ), 'Expected presence_members to be a non-negative int'
        assert (
            isinstance(channel_status.status.occupancy.metrics.presence_connections, int)
            and channel_status.status.occupancy.metrics.presence_connections >= 0
        ), 'Expected presence_connections to be a non-negative int'
        assert (
            isinstance(channel_status.status.occupancy.metrics.presence_subscribers, int)
            and channel_status.status.occupancy.metrics.presence_subscribers >= 0
