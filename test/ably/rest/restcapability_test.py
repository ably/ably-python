...
class TestRestCapability(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    async def test_equal_intersection_with_key(self):
        ...
        token_details = await self.ably.auth.request_token(
            key_name=key['key_name'], key_secret=key['key_secret'], token_params={'capability': key['capability']}
        )
    ...
    async def test_non_empty_ops_intersection(self):
        ...
        token_params = {'capability': {'channel2': ['presence', 'subscribe']}}
        kwargs = {
            'key_name': key['key_name'],
            'key_secret': key['key_secret'],
        }

        expected_capability = Capability({'channel2': ['subscribe']})
    ...
    async def test_non_empty_paths_intersection(self):
        ...
        kwargs = {'key_name': key['key_name'], 'key_secret': key['key_secret']}
    ...
    async def test_wildcard_ops_intersection(self):
        ...
        expected_capability = Capability({'channel2': ['subscribe', 'publish']})
    ...
    async def test_wildcard_ops_intersection_2(self):
        ...
        expected_capability = Capability({'channel6': ['subscribe', 'publish']})
    ...
    async def test_wildcard_resources_intersection(self):
        ...
        expected_capability = Capability({'cansubscribe': ['subscribe']})
    ...
    async def test_wildcard_resources_intersection_2(self):
        ...
        expected_capability = Capability({'cansubscribe:check': ['subscribe']})
    ...
    async def test_invalid_capabilities(self):
        with pytest.raises(AblyException) as excinfo:
            await self.ably.auth.request_token(token_params={'capability': {'channel0': ['publish_']}})
    ...
    async def test_invalid_capabilities_2(self):
        with pytest.raises(AblyException) as excinfo:
            await self.ably.auth.request_token(token_params={'capability': {'channel0': ['*', 'publish']}})
    ...
    async def test_invalid_capabilities_3(self):
        with pytest.raises(AblyException) as excinfo:
            await self.ably.auth.request_token(token_params={'capability': {'channel0': []}})
