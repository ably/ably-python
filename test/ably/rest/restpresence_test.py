...
class TestPresence(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    async def test_get_presence_with_limit(self):
        ...
        assert "limit=300" in responses.calls[0].request.url
    ...
    async def test_history_with_direction(self):
        ...
        assert "direction=backwards" in responses.calls[0].request.url
    ...
    async def test_with_milisecond_start_end(self):
        ...
        assert "start=100000" in responses.calls[0].request.url
        assert "end=100001" in responses.calls[0].request.url
    ...
    async def test_with_timedate_startend(self):
        ...
        assert f"start={start_ms}" in responses.calls[0].request.url
        assert f"end={end_ms}" in responses.calls[0].request.url
