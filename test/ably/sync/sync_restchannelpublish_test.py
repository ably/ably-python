...
class TestRestChannelPublish(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    def setUp(self):
        ...
        self.ably_with_client_id = TestAppSync.get_ably_rest(client_id=self.client_id, use_token_auth=True)
    ...
    def test_wildcard_client_id_can_publish_as_others(self):
        ...
        wildcard_ably = TestAppSync.get_ably_rest(
            key=None,
...
class TestRestChannelPublishIdempotent(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    def setUp(self):
        self.ably = TestAppSync.get_ably_rest()
        self.ably_idempotent = TestAppSync.get_ably_rest(idempotent_rest_publishing=True)
    ...
    def test_idempotent_rest_publishing(self):
        ...
        ably = TestAppSync.get_ably_rest(idempotent_rest_publishing=True)
        assert ably.options.idempotent_rest_publishing is True
        ably.close()

        ably = TestAppSync.get_ably_rest(idempotent_rest_publishing=False)
