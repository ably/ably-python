...
class TestRestAppStatsSetup:
    ...
    def setUp(self):
        ...
        self.ably_text = TestAppSync.get_ably_rest(use_binary_protocol=False)
        ...
        if TestRestAppStatsSetup.__stats_added:
            return
        self.ably.http.post('/stats', body=stats + previous_stats)
        TestRestAppStatsSetup.__stats_added = True

    def tearDown(self):
        self.ably.close()
        self.ably_text.close()
...
class TestRestAppStats(TestRestAppStatsSetup, BaseAsyncTestCase,
    ...
    def test_protocols(self):
        stats_pages = self.ably.stats(**self.get_params())
        stats_pages1 = self.ably_text.stats(**self.get_params())
