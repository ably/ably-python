...
from test.ably.sync.utils import new_dict, random_string, get_random_key
...
class TestPush(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    def setUp(self):
        ...
        for key, channel in zip(self.devices, itertools.cycle(self.channels)):
            ...
            self.save_subscription(channel, device_id=device.id)
        assert len(list(itertools.chain(*self.channels.values()))) == len(self.devices)

    def tearDown(self):
        for key, channel in zip(self.devices, itertools.cycle(self.channels)):
            device = self.devices[key]
            self.remove_subscription(channel, device_id=device.id)
            self.ably.push.admin.device_registrations.remove(device_id=device.id)
        self.ably.close()
    ...
    def save_subscription(self, channel, **kw):
        """
        Helper method to register a device, to not have this code repeated
        everywhere. Returns the input dict that was sent to Ably, and the
        device details returned by Ably.
        ...
        subscription = self.ably.push.admin.channel_subscriptions.save(subscription)
        self.channels.setdefault(channel, []).append(subscription)
        return subscription

    def remove_subscription(self, channel, **kw):
        subscription = PushChannelSubscription(channel, **kw)
        subscription = self.ably.push.admin.channel_subscriptions.remove(subscription)
        return subscription
    ...
    def test_admin_device_registrations_list(self):
        ...
        # limit
        list_response = list_devices(limit=5000)
        assert len(list_response.items) == len(self.devices)
        list_response = list_devices(limit=2)
        assert len(list_response.items) == 2

        # Filter by device id
        device = self.get_device()
        list_response = list_devices(deviceId=device.id)
        assert len(list_response.items) == 1
        list_response = list_devices(deviceId=self.get_device_id())
        assert len(list_response.items) == 0

        # Filter by client id
        list_response = list_devices(clientId=device.client_id)
        assert len(list_response.items) == 1
        list_response = list_devices(clientId=self.get_client_id())
        assert len(list_response.items) == 0
    ...
    def test_admin_device_registrations_remove(self):
        ...
        # Remove
        get_response = get(device.id)
        assert get_response.id == device.id  # Exists
        remove_device_response = self.remove_device(device.id)
    ...
    def test_admin_device_registrations_remove_where(self):
        ...
        # Remove by device id
        device = self.get_device()
        foo_device = get(device.id)
        assert foo_device.id == device.id  # Exists
        remove_foo_device_response = self.remove_device_where(deviceId=device.id)
        ...
        # Remove by client id
        device = self.get_device()
        boo_device = get(device.id)
        assert boo_device.id == device.id  # Exists
        remove_boo_device_response = self.remove_device_where(clientId=device.client_id)
    ...
    def test_admin_channel_subscriptions_list(self):
        ...
        # limit
        list_response = list_(channel=channel, limit=2)
        assert len(list_response.items) == 2

        list_response = list_(channel=channel, limit=5000)
        assert len(list_response.items) == len(subscriptions)

        # Filter by device id
        device_id = subscriptions[0].device_id
        list_response = list_(channel=channel, deviceId=device_id)
        assert len(list_response.items) == 1
        assert list_response.items[0].device_id == device_id
        assert list_response.items[0].channel == channel
        list_response = list_(channel=channel, deviceId=self.get_device_id())
        assert len(list_response.items) == 0

        # Filter by client id
        device = self.get_device()
        list_response = list_(channel=channel, clientId=device.client_id)
        assert len(list_response.items) == 0
    ...
    def test_admin_channels_list(self):
        ...
        # limit
        list_response = list_(limit=5000)
        assert len(list_response.items) == len(self.channels)
        list_response = list_(limit=1)
        assert len(list_response.items) == 1
    ...
    def test_admin_channel_subscriptions_remove(self):
        ...
        remove_response = remove(subscription)
    ...
    def test_admin_channel_subscriptions_remove_where(self):
        ...
        list_response = list_(channel=channel)
        assert device.id in (x.device_id for x in list_response.items)
        remove_response = remove(channel=channel, device_id=device.id)
        assert remove_response.status_code == 204
        list_response = list_(channel=channel)
        assert device.id not in (x.device_id for x in list_response.items)
        ...
        list_response = list_(channel=channel)
        assert client_id in (x.client_id for x in list_response.items)
        remove_response = remove(channel=channel, client_id=client_id)
        assert remove_response.status_code == 204
        list_response = list_(channel=channel)
        assert client_id not in (x.client_id for x in list_response.items)
