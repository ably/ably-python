...
from ably.sync import Capability
...
class TestRestToken(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    def test_token_generation_with_local_time(self):
        ...
        with patch('ably.sync.rest.rest.AblyRestSync.time', wraps=self.ably.time) as server_time,\
...
class TestCreateTokenRequest(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    def test_key_name_and_secret_are_required(self):
        ably = TestApp.get_ably_rest(key=None, token='not a real token')
    ...
    @dont_vary_protocol
    def test_hmac(self):
        ably = AblyRestSync(key_name='a_key_name', key_secret='a_secret')
        token_params = {
            'ttl': 1000,
            'nonce': 'abcde100',
            'client_id': 'a_id',
