...
class TestAuthAuthorize(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    def test_timestamp_is_not_stored(self):
        ...
        with mock.patch('ably.sync.rest.auth.TokenRequest',
...
class TestRequestToken(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    def test_with_key(self):
        ably = TestApp.get_ably_rest(use_binary_protocol=self.use_binary_protocol)
    ...
    @dont_vary_protocol
    @respx.mock
    def test_with_auth_url_headers_and_params_http_post(self):  # noqa: N802
        url = 'http://www.example.com'
        headers = {'foo': 'bar'}
        ably = TestApp.get_ably_rest(key=None, auth_url=url)
    ...
    @dont_vary_protocol
    @respx.mock
    def test_with_auth_url_headers_and_params_http_get(self):  # noqa: N802
        url = 'http://www.example.com'
        headers = {'foo': 'bar'}
        ably = TestApp.get_ably_rest(
    ...
    def test_with_callback(self):
        ...
        ably = TestApp.get_ably_rest(key=None, auth_callback=callback)
    ...
    def test_client_id_null_until_auth(self):
        client_id = uuid.uuid4().hex
        token_ably = TestApp.get_ably_rest(
...
class TestRenewToken(BaseAsyncTestCase):
    ...
    def setUp(self):
        ...
        self.ably = TestApp.get_ably_rest(use_binary_protocol=False, rest_host=self.host)
        # with headers
...
class TestRenewExpiredToken(BaseAsyncTestCase):
    ...
    def test_query_time_false(self):
        ably = TestApp.get_ably_rest(rest_host=self.host)
        ably.auth.authorize()
    ...
    def test_query_time_true(self):
        ably = TestApp.get_ably_rest(query_time=True, rest_host=self.host)
        ably.auth.authorize()
        self.publish_fail = False
        ably.channels[self.channel].publish('evt', 'msg')
        assert self.publish_attempts == 1
