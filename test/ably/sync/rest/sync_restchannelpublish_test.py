...
from test.ably.sync import utils
...
class TestRestChannelPublish(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    def setUp(self):
        ...
        self.ably_with_client_id = TestApp.get_ably_rest(client_id=self.client_id, use_token_auth=True)

    def tearDown(self):
        self.ably.close()
        self.ably_with_client_id.close()
    ...
    @dont_vary_protocol
    def test_unsupported_payload_must_raise_exception(self):
    ...
    def test_wildcard_client_id_can_publish_as_others(self):
        ...
        wildcard_ably = TestApp.get_ably_rest(
            key=None,
...
class TestRestChannelPublishIdempotent(BaseAsyncTestCase, metaclass=VaryByProtocolTestsMetaclass):
    ...
    def setUp(self):
        self.ably = TestApp.get_ably_rest()
        self.ably_idempotent = TestApp.get_ably_rest(idempotent_rest_publishing=True)

    def tearDown(self):
        self.ably.close()
        self.ably_idempotent.close()
    ...
    def test_idempotent_rest_publishing(self):
        ...
        ably = TestApp.get_ably_rest(idempotent_rest_publishing=True)
        assert ably.options.idempotent_rest_publishing is True
        ably.close()

        ably = TestApp.get_ably_rest(idempotent_rest_publishing=False)
