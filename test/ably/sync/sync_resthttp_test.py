...
class TestRestHttp(BaseAsyncTestCase):
    ...
    def test_cached_fallback(self):
        timeout = 2000
        ably = TestAppSync.get_ably_rest(fallback_retry_timeout=timeout)
    ...
    def test_add_request_ids(self):
        # With request id
        ably = TestAppSync.get_ably_rest(add_request_ids=True)
        r = ably.http.make_request('HEAD', '/time', skip_auth=True)
    ...
    def test_request_over_http2(self):
        ...
        ably = TestAppSync.get_ably_rest(rest_host=url)
