...
class TestRealtimeAuth(BaseAsyncTestCase):
    ...
    async def test_auth_with_auth_url_post(self):
        ...
        ably = await TestApp.get_ably_realtime(auth_url=url_path, auth_method='POST', auth_params=token_details)
        await ably.connection.once_async(ConnectionState.CONNECTED)
        response_time_ms = await ably.connection.ping()
        assert response_time_ms is not None
        assert ably.connection.error_reason is None
    ...
    async def test_jwt_reauth(self):
        ...
        async def auth_callback(_):
            response = httpx.get(
                echo_url + '/createJWT', params={'keyName': key_name, 'keySecret': key_secret, 'expiresIn': 35}
            )
    ...
    async def test_renew_token_single_attempt(self):
        ...
        ably = await TestApp.get_ably_realtime(auth_callback=callback)
        msg = {'action': ProtocolMessageAction.ERROR, 'error': {'code': 40142, 'statusCode': 401}}
    ...
    async def test_auth_callback_error(self):
        ...
        await auth_callback_failure({'auth_callback': auth_callback})
    ...
    async def test_auth_url_timeout(self):
        await auth_callback_failure({'auth_url': 'http://10.255.255.1/'})

    async def test_auth_url_404(self):
        await auth_callback_failure({'auth_url': 'http://example.com/404'})

    async def test_auth_url_wrong_content_type(self):
        await auth_callback_failure({'auth_url': 'http://example.com/'})

    async def test_auth_url_401(self):
        await auth_callback_failure({'auth_url': echo_url + '/respondwith?status=401'})

    async def test_auth_url_403(self):
        await auth_callback_failure({'auth_url': echo_url + '/respondwith?status=403'}, expect_failure=True)
    ...
    async def test_renew_token_single_attempt_upon_disconnection(self):
        ...
        ably = await TestApp.get_ably_realtime(auth_callback=callback)
        msg = {'action': ProtocolMessageAction.DISCONNECTED, 'error': {'code': 40142, 'statusCode': 401}}
